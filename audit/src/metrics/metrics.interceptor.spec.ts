...
function httpCtx(opts: { method?: string; url?: string; route?: string | null; statusCode?: number } = {}) {
  return {
    switchToHttp: () => ({
      getRequest: () => ({
...
describe('MetricsInterceptor', () => {
  it('records a successful HTTP request with route and status', async () => {
    ...
    await lastValueFrom(interceptor.intercept(httpCtx({ statusCode: 200 }), { handle: () => of('ok') }));
  ...
  it('falls back to route unknown when neither routeOptions nor url is present', async () => {
    const metrics = { recordRequest: jest.fn() };
    const interceptor = new MetricsInterceptor(metrics as unknown as MetricsService);
    const ctx = {
      switchToHttp: () => ({
        getRequest: () => ({ method: 'GET', url: undefined, routeOptions: {} }),
    ...
    expect(metrics.recordRequest).toHaveBeenCalledWith('GET', 'unknown', 200, expect.any(Number));
  });
});
