...
async function bootstrap() {
  ...
  installProcessHandlers({ logger, service: 'company' });
  ...
  const fromCwd = join(process.cwd(), '..', 'proto', 'fairflow', 'company', 'v1', 'company.proto');
  const fromCwdRoot = join(process.cwd(), 'proto', 'fairflow', 'company', 'v1', 'company.proto');
  ...
  logger.info({ grpcPort: config.grpcPort }, 'Company gRPC started');
