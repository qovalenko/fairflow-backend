...
/**
 * Mongo-backed transactional outbox store (E3-01, RFC-4 §Р-4; mirrors the
 * contact reference impl).
 *
 * `withOutbox` is the write half: it runs the business mutation and inserts the
 * outbox rows it returns in ONE Mongo session, so the business record and the
 * event row commit or roll back together (invariant Д-1). The relay half reads
 * pending rows and flips their status.
 */
@Injectable()
