...
/**
 * Background relay loop: polls the Mongo outbox for `pending` rows and publishes
 * them to RabbitMQ at-least-once. Broker/Mongo hiccups just leave rows `pending`.
 * Disable with `OUTBOX_RELAY_ENABLED=false`.
 */
@Injectable()
export class OutboxRelayService implements OnModuleInit, OnModuleDestroy {
  ...
  private async runOnce(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`outbox relay tick failed: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      this.running = false;
