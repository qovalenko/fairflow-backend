...
/**
 * Mongo-backed transactional outbox store (RFC-4 §Р-4). `enqueue` inserts an
 * outbox row in the SAME Mongo session as the business mutation so the chat
 * fact and its `chat.*` event commit/abort together (after-commit, at-least-once,
 * NFR-CHAT-5). The relay reads pending rows and flips their status.
 */
@Injectable()
export class MongoOutboxStore implements OutboxStore {
   ...
   * ONE transaction. On a standalone dev Mongo (no tx support) it falls back to
   * business-write-then-outbox (best-effort window).
   */
  async withOutbox<R>(
