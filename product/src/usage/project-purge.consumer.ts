 ...
 * `crm_product_usage_processed` is the listener dedup-ledger (scoped by project);
 * clearing it on purge is harmless (the project's usage facts are moot).
 * NOT included: `crm_event_outbox` (events in flight).
 */
const PURGE_COLLECTIONS = ['crm_products', 'crm_product_usage_processed'] as const;
 ...
 * of the product domain on a project hard-purge.
 *
...
export class ProjectPurgeConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('product.project-purge');
    try {
