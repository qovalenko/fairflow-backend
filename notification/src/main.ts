...
async function bootstrap() {
  ...
  installProcessHandlers({ service: 'notification' });

  const app = await NestFactory.create(AppModule);
  const grpcPort = parseInt(process.env.GRPC_NOTIFICATION_PORT ?? '5015', 10);
  ...
  const httpPort = parseInt(process.env.HTTP_PORT ?? '3015', 10);
  await app.listen(httpPort);
  console.log(`Notification gRPC on 0.0.0.0:${grpcPort}; HTTP health on ${httpPort}`);
}
