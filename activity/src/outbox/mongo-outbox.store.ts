...
/**
 * Mongo-backed transactional outbox store (E3-01 reference impl, RFC-4 §Р-4).
 *
 * `enqueue` is the write half: it inserts an outbox row in the SAME Mongo
 * session as the business mutation, so the business record and the event row
 * commit or roll back together (invariant Д-1). The relay half reads pending
 * rows and flips their status.
 */
@Injectable()
export class MongoOutboxStore implements OutboxStore {
  ...
  async withOutbox<R>(
    ...
    const coll = await this.mongo.outbox();
  ...
  /**
   * NFR-ACT-070: periodically requeue `failed` rows after broker outages so
   * at-least-once delivery is restored without operator intervention.
   */
  async requeueStaleFailed(at: Date, staleAfterMs: number, limit: number): Promise<number> {
    const coll = await this.mongo.outbox();
    const cutoff = new Date(at.getTime() - staleAfterMs);
    const cap = Math.max(1, Math.min(limit, 500));
    const docs = await coll
      .find({ status: 'failed', updatedAt: { $lte: cutoff } })
      .sort({ updatedAt: 1 })
      .limit(cap)
      .toArray();
    if (!docs.length) return 0;
    const ids = docs.map((d) => d.messageId);
    const res = await coll.updateMany(
      { messageId: { $in: ids }, status: 'failed' },
      { $set: { status: 'pending', attempts: 0, updatedAt: at }, $unset: { lastError: '' } },
    );
