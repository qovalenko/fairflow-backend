...
/**
 * Consumer of `control.member.offboarded` (BX-OFFB-2): reassigns every activity
 * the departed member owned (in the event's project) to the chosen active
 * responsible. Per-record `crm.activity.reassigned` events (so notification/denorm
 * never drift) are emitted by `ActivityService.reassignOwnedRecords`.
 *
...
export class MemberOffboardedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('activity.member-offboarded');
    try {
  ...
  /**
   * Reassign the leaver's activities in this project. Throws on a Mongo error
  ...
  async handle(payload: Record<string, unknown>): Promise<OffboardReassignOutcome> {
    ...
    // idempotency key is `activity.reassigned:<id>:<offboardTs>`, so a stable `:0`
    ...
    const { reassigned } = await this.activity.reassignOwnedRecords(
      projectId,
      from,
      to,
      offboardTs,
    ...
    this.logger.log(
      `offboard reassign project=${projectId} ${from}→${to}: activities=${reassigned}`,
    );
