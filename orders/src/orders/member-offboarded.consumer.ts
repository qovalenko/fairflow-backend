...
/**
 * Consumer of `control.member.offboarded` (BX-OFFB-2): reassigns every order the
 * departed member owned (in the event's project) to the chosen active
 * responsible. Per-record `crm.order.status_changed` events (so reports/denorm
 * never drift) are emitted by `OrdersService.reassignOwnedRecords`.
 *
 * Isolation: the reassign filter is `{ projectId, assigneeId: from }` from the
 * envelope. Idempotency is natural (a redelivery finds nothing still owned by
 * `from`). A poison message (missing projectId / from / to) is terminal
 * (`dead_letter`); a Mongo error throws → retry ladder / DLQ.
 */
@Injectable()
export class MemberOffboardedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('orders.member-offboarded');
    try {
  ...
  /**
   * Reassign the leaver's orders in this project. Throws on a Mongo error
  ...
  async handle(payload: Record<string, unknown>): Promise<OffboardReassignOutcome> {
    ...
    // idempotency key is `order.reassigned:<id>:<offboardTs>`, so a stable `:0`
    ...
    const { reassigned } = await this.orders.reassignOwnedRecords(projectId, from, to, offboardTs);
    this.logger.log(`offboard reassign project=${projectId} ${from}→${to}: orders=${reassigned}`);
