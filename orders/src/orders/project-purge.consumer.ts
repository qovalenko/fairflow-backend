 ...
 * NOT included: any `*_outbox` (events in flight), `idempotency_keys` (TTL).
 */
const PURGE_COLLECTIONS = [
  'crm_order_types',
  'crm_orders',
  'crm_order_counters',
  'crm_order_type_revisions',
] as const;
 ...
 * of the orders domain (order types, orders, counters, type revisions) on a
 * project hard-purge.
 *
...
export class ProjectPurgeConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('orders.project-purge');
    try {
