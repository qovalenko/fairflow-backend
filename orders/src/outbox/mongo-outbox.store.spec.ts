...
describe('MongoOutboxStore', () => {
  function makeStore(
    ...
    const insertOne = opts.insertOne ?? jest.fn(async () => undefined);
    const coll = {
      insertMany,
      insertOne,
      find:
    ...
    return { store: new MongoOutboxStore(mongo), coll, session, insertMany, insertOne };
  }
  ...
  it('withOutbox в транзакции пишет outbox-строки вместе с business result', async () => {
    ...
    const result = await store.withOutbox(async () => ({
      result: 'ok',
      intents: [{ type: 'crm.order.created', source: 'orders', payload: {} }],
    }));
  ...
  it('withOutbox fallback без транзакций (standalone Mongo)', async () => {
    ...
    const result = await store.withOutbox(async () => ({
      result: 42,
      intents: [{ type: 'crm.order.status_changed', source: 'orders', payload: {} }],
    }));
  ...
  it('enqueue вставляет одну outbox-строку в активную сессию', async () => {
    const insertOne = jest.fn(async () => undefined);
    const session = { id: 'sess-1' };
    const { store } = makeStore({ insertOne });
    const row = await store.enqueue(
      { type: 'crm.order.created', source: 'orders', payload: { orderId: 'o1' } },
      session as never,
    );
    expect(row.messageId).toBeDefined();
    expect(insertOne).toHaveBeenCalledWith(expect.objectContaining({ status: 'pending' }), {
      session,
    });
  });
