...
/**
 * Mongo-backed transactional outbox store for the documents domain (E3-01
 * reference impl, RFC-4 §Р-4). Mirrors the pipe/contact implementation.
 *
 * `withOutbox` is the write half: it runs the business mutation and inserts the
 * resulting outbox rows in the SAME Mongo session, so the business record and
 * its event row commit/abort together (invariant Д-1). On standalone Mongo (dev,
 * no replica set → no multi-doc TX) it falls back to a sequential write that
 * narrows but does not eliminate the gap. The relay half reads pending rows and
 * flips their status; consumers dedup on the envelope `idempotencyKey`.
 */
@Injectable()
export class MongoOutboxStore implements OutboxStore {
   ...
   * ONE transaction. `work` receives the active session (pass it to your
   * `insertOne`/`updateOne`) and returns `{ result, intents }`.
   */
  async withOutbox<R>(
  ...
  /**
   * Build + insert a single outbox row inside `session` (the caller's business
   * tx). Validates the routing-key against the RFC-4 §Р-3 registry (throws →
