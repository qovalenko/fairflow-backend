...
/**
 * Consumer of `control.member.offboarded` (BX-OFFB-2): reassigns every document
 * group the departed member owned (in the event's project) to the chosen active
 * responsible. The heavy lifting lives in `DocumentsService.reassignOwnedRecords`.
 */
@Injectable()
export class MemberOffboardedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('documents.member-offboarded');
    try {
  ...
  async handle(payload: Record<string, unknown>): Promise<OffboardReassignOutcome> {
    ...
    const { reassigned } = await this.documents.reassignOwnedRecords(
      projectId,
      from,
      to,
      offboardTs,
    );
    this.logger.log(`offboard reassign project=${projectId} ${from}→${to}: documents=${reassigned}`);
