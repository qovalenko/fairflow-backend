...
/**
 * Inbound RabbitMQ consumer for the reports statistics-rollup listener (P2.f).
 *
 ...
 * are exhausted — never a silent `nack(requeue=false)` drop. This is a verbatim
 * sibling of the documents drift consumer (same shared bus-topology helpers).
 *
 ...
 */
@Injectable()
export class ReportsRabbitMqConsumer implements OnModuleDestroy {
  private readonly logger = new Logger(ReportsRabbitMqConsumer.name);
  ...
  private async getChannel(): Promise<Channel> {
    ...
    connection.on('error', (err) =>
      this.logger.error(`rollup consumer connection error: ${String(err)}`),
    );
    connection.on('close', () => this.onDisconnect('connection closed'));

    const channel = await connection.createChannel();
    channel.on('error', (err) =>
      this.logger.error(`rollup consumer channel error: ${String(err)}`),
    );
  ...
  private async handleMessage(
    ...
    try {
      ...
      if (decision.kind === 'retry') {
        this.logger.warn(
          `rollup event failed (attempt ${attempt + 1}/${topology.maxAttempts}), ` +
        ...
        this.logger.error(
          `rollup event dead-lettered after ${topology.maxAttempts} attempts: ${String(error)}`,
        );
  ...
  private onDisconnect(reason: string): void {
    ...
    this.logger.warn(`rollup consumer disconnected (${reason}) — scheduling reconnect`);
  ...
  private scheduleReconnect(): void {
    ...
    const delay = Math.min(
      ReportsRabbitMqConsumer.RECONNECT_BASE_MS * 2 ** this.reconnectAttempt,
      ReportsRabbitMqConsumer.RECONNECT_MAX_MS,
    );
  ...
  private async reconnect(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`rollup consumer reconnected (${this.subscriptions.length} subscription(s))`);
    } catch (err) {
      this.closeQuietly(this.connection, this.channel);
      this.channel = null;
      this.connection = null;
      this.logger.error(`rollup consumer reconnect failed: ${String(err)} — retrying`);
      this.scheduleReconnect();
    }
  }
