...
/**
 * Consumer of `control.member.offboarded` (BX-OFFB-2): reassigns every deal the
 * departed member owned (in the event's project) to the chosen active
 * responsible. Per-record `crm.deal.reassigned` events (so search/denorm never
 * drift) are emitted by `PipeService.reassignOwnedRecords`.
 *
...
export class MemberOffboardedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('pipe.member-offboarded');
    try {
  ...
  /**
   * Reassign the leaver's deals in this project. Throws on a Mongo error
  ...
  async handle(payload: Record<string, unknown>): Promise<OffboardReassignOutcome> {
    ...
    // idempotency key is `deal.reassigned:<id>:<offboardTs>`, so a stable `:0`
    ...
    const { reassigned } = await this.pipe.reassignOwnedRecords(projectId, from, to, offboardTs);
    this.logger.log(`offboard reassign project=${projectId} ${from}→${to}: deals=${reassigned}`);
