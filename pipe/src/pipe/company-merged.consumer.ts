 ...
 * open deal in the project that still points at a merge-tombstone loser company.
 */
@Injectable()
export class CompanyMergedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('pipe.company-merged');
    try {
  ...
  async handle(payload: Record<string, unknown>): Promise<CompanyMergedOutcome> {
    ...
    const { rewritten } = await this.pipe.rewriteCompanyOnMerge(
      projectId,
      loserId,
      masterId,
      mergeKey,
    ...
    if (rewritten > 0) {
      this.logger.log(
        `company merge project=${projectId} ${loserId}→${masterId}: rewrote ${rewritten} deal(s)`,
      );
    }
