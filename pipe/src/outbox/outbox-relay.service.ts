...
export class OutboxRelayService implements OnModuleInit, OnModuleDestroy {
  ...
  private async runOnce(): Promise<void> {
    ...
    try {
      ...
      if (result.fetched > 0) this.metrics?.recordOutboxRelay('fetched', result.fetched);
      if (result.published > 0) this.metrics?.recordOutboxRelay('published', result.published);
      if (result.failed > 0) this.metrics?.recordOutboxRelay('failed', result.failed);
