...
function authProtoPath(): string {
  const fromDist = join(__dirname, '..', '..', '..', 'proto', 'fairflow', 'auth', 'v1', 'auth.proto');
...
@Module({
  ...
  providers: [GatewayApiKeyValidationService, { provide: APP_GUARD, useClass: GrpcInboundApiKeyGuard }],
