...
describe('HealthController', () => {
  it('healthz returns ok without touching mongo', () => {
    ...
    expect(mongo.healthPing).not.toHaveBeenCalled();
  });

  it('readyz returns 200 when mongo responds', async () => {
  ...
  it('status exposes the shared ops payload', () => {
    ...
    expect(controller.status()).toEqual(expect.objectContaining({ service: expect.any(String) }));
  });
});
