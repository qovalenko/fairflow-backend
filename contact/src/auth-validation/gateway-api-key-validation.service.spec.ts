...
describe('GatewayApiKeyValidationService', () => {
  ...
  it('fail-opens on transport error when a positive cache entry is within stale grace', async () => {
    ...
    // Soft TTL expired, stale-grace still open — must re-ask auth, then fail-open.
    jest.setSystemTime(1_700_000_000_000 + 5_000);
  ...
  it('surfaces UNAVAILABLE when stale grace has expired and auth is down', async () => {
    jest.useFakeTimers();
    jest.setSystemTime(1_700_000_000_000);
    process.env.GATEWAY_KEY_CACHE_TTL_MS = '1000';
    process.env.GATEWAY_KEY_STALE_GRACE_MS = '2000';
    const { svc, validateServiceApiKey } = build(true);
    await svc.assertValidGatewayCall(validMetadata());
