 ...
 * NOT included: `_outbox` (events in flight), `idempotency_keys` (self-pruning TTL).
 */
const PURGE_COLLECTIONS = ['contacts'] as const;
 ...
 * of the contact domain on a project hard-purge.
 *
 * Isolation: the delete filter is `{ projectId }` from the envelope — a foreign
 * event can never touch another tenant's data. Idempotency is natural (a
 * redelivery deletes 0 rows), so no dedup-inbox is needed. A Mongo error throws →
 * shared retry ladder / DLQ; a poison message (no projectId) is terminal
 * (`dead_letter`) — logged and acked, never retried forever.
 */
@Injectable()
export class ProjectPurgeConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('contact.project-purge');
    try {
