...
/**
 * Consumer of `crm.company.merged` (FR-COMPANIES-140): rewrites M2M company links on
 * every contact in the project that still references the merge-tombstone loser.
 */
@Injectable()
export class CompanyMergedConsumer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = busQueueName('contact.company-merged');
    try {
  ...
  async handle(payload: Record<string, unknown>): Promise<CompanyMergedOutcome> {
    ...
    const { rewritten } = await this.contacts.rewriteCompanyOnMerge(
      projectId,
      loserId,
      masterId,
      mergeKey,
    ...
    if (rewritten > 0) {
      this.logger.log(
        `company merge project=${projectId} ${loserId}→${masterId}: rewrote ${rewritten} contact(s)`,
      );
    }
